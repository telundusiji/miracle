 ...
 * <p>
 * It also support embedded JSON format; Use VehicleJsonStreamParser#EMBEDDED_PROPERTY_SEPARATOR) to separate them and save into
 ...
 */
public final class VehicleJsonStreamParser extends StreamingParser {

    private static final Logger logger = LoggerFactory.getLogger(VehicleJsonStreamParser.class);
    private static final String PROPERTY_DATA = "values";
    ...
    private final Map<String, Integer> mappingMap = new HashMap<>();
    ...
    public VehicleJsonStreamParser(List<TblColRef> allColumns, Map<String, String> properties) {
        ...
        mapping = properties.get(PROPERTY_MAPPING);
        if (StringUtils.isEmpty(mapping)) {
            mapping = "0:passTime,4:crossingId,5:laneNo,11:vehicleType,28:vehicleLogo,62:plateBelong,71:plateProvince";
        }
        try {
            String[] mappings = mapping.split(",");
            for (String map : mappings) {
                String[] maps = map.split(":");
                mappingMap.put(maps[1], Integer.valueOf(maps[0]));
            }
        } catch (Exception e) {
            throw new IllegalStateException("Invalid StreamingConfig, mapping " + mapping + ", parserProperties " + properties + ".[e.g.]\"index1:field1,index2:field2\"", e);
        }
    ...
    public List<StreamingMessageRow> parse(ByteBuffer buffer) {
        try {
            ...
            logger.info(charset.decode(buffer).toString());
            Map<String, Object> message = mapper.readValue(new ByteBufferBackedInputStream(buffer), mapType);
            root.clear();
            root.putAll(message);
            List values = Arrays.asList(root.get(PROPERTY_DATA));
            String tsStr = objToString(values.get(mappingMap.get(tsColName)));
            long t = streamTimeParser.parseTime(tsStr);
            Map<String, Object> rootMap = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
            for (Map.Entry<String, Integer> entry : mappingMap.entrySet()) {
                rootMap.put(entry.getKey(), values.get(entry.getValue()));
            }
            rootMap.put(tsColName, t);
            ArrayList<String> result = Lists.newArrayList();
            logger.info(String.valueOf(rootMap));
            logger.info(String.valueOf(values));
            logger.info(String.valueOf(t));
            logger.info(String.valueOf(derivedTimeColumns));
            ...
            for (TblColRef column : allColumns) {
                ...
                if (populateDerivedTimeColumns(columnName, result, t) == false) {
                    result.add(getValueByKey(column, rootMap));
                }
            }
            ...
            Charset charset = Charset.forName("utf-8");
            logger.info(charset.decode(buffer).toString());
            logger.error(charset.decode(buffer).toString(), e);
